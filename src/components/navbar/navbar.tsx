...
function Navbar() {
  ...
  const navigationItems = [
    ...
    isAuthenticated ? { label: "Viagens", path: "/" } : null, // Condicional para "Viagens"
    isAuthenticated ? { label: "Perfil", path: "/perfil" } : null, // Condicional para "Perfil"
    { label: "Sobre", path: "/sobre" },
  ].filter(Boolean); // Remove os itens null se não estiver autenticado
  ...
  return (
    <header className="flex bg-gradient-custom font-roboto w-full p-4 items-center justify-between sticky top-0 z-50">
      <Link to={navigationItems[0].path} className="font-extrabold flex items-center gap-1 text-black hover:text-gray-100 transition-colors">
        <img src={Logo} alt="Velo" className="w-10 transform transition-transform duration-300 hover:scale-110" />
        <h1 className="text-2xl hidden sm:block">VELO</h1>
      </Link>

      <nav className="font-bold sm:ml-auto flex items-center gap-5 text-black">
        <button
        ...
        >
          <div className="flex mb-20 items-center">
            <img src={Logo} alt="Velo" className="w-10 transform transition-transform duration-300 hover:scale-110" />
            <h1 className="flex text-2xl block ml-2 text-black">VELO</h1>
          </div>
        ...
        {navigationItems.map((item) => (
          <Link 
            key={item.path} 
            to={item.path} 
            className="sm:flex hidden text-black hover:text-gray-100 transition-colors"
          >
            {item.label}
      ...
      <div className="ml-10 flex gap-4 items-center font-bold sm:ml-10 ml-auto">
        {isAuthenticated ? (
          <>
            <div 
